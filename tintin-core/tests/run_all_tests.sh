...
PUBLIC_DIR="$(cd "${ROOT_DIR}/../public" && pwd)"
...
run_step "PHP lint (api/includes)" bash -lc "find '${PUBLIC_DIR}/api' '${ROOT_DIR}/includes' -name '*.php' -print0 | xargs -0 -n1 php -l >/dev/null"
...
run_step "E2E: Forwarded delete safety" bash tests/e2e/forwarded_delete_safety_smoke.sh "${BASE_URL}"
...
run_step "E2E: Admin media cleanup" bash tests/e2e/admin_media_cleanup_smoke.sh "${BASE_URL}"
...
run_step "E2E: Conversation search navigation" bash tests/e2e/conversation_search_navigation_smoke.sh
