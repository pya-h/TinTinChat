...
function apiGuardOversizedPostBody(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST) && empty($_FILES) && $contentLength > 0) {
        ...
        apiError(
            'UPLOAD_TOO_LARGE',
            "Uploaded data exceeds server limit (post_max_size: {$postMaxSize}, upload_max_filesize: {$uploadMaxSize}). Restart local server from the public/ docroot with: php -d upload_max_filesize=110M -d post_max_size=120M -S localhost:8080",
            400
        );
    }
}
