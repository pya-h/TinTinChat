...
function renderIdeasList(ideas, pagination) {
    ...
    const isSuperuser = Boolean(CURRENT_USER_IS_SUPERUSER);
    for (const idea of ideas) {
        ...
        // Only superuser sees author names; own ideas show "You"
...
async function voteOnIdea(ideaId, vote) {
    // Optimistic in-place update + server sync
    ...
    try {
        const result = await window.ApiService.jsonOk("api/ideas/vote.php", {
            method: "POST",
            headers: { "Content-Type": "application/json", ...getCsrfHeaders() },
            body: JSON.stringify({ idea_id: ideaId, vote }),
        });
        // In-place update if card still exists
        if (card && result) {
